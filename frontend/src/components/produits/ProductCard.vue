...
const mainImage = computed(() => {
  return (
    props.produit.photos?.[0] ||
    props.produit.photo_url ||
  ...
  >
    <div :to="`/produits/${produit.id}`" class="block relative">
      ...
      <div class="relative w-full h-30 overflow-hidden bg-gray-50">
        ...
        >
          <div
            v-for="(photo, index) in produit.photos || [mainImage]"
            :key="index"
        ...
        <div class="absolute top-3 left-3 flex flex-col gap-1">
          <span
            ...
            class="text-[var(--espace-vert)]/50 text-sm font-semibold rounded-full shadow-lg flex items-center g"
          >
            <i class="sm:flex fas fa-handshake text-[10px]"></i>
          </span>
          <span
            v-if="produit.boosted"
            class="bg-yellow-500 text-green-900 text-sm font-semibold px-2 py-1 rounded-full shadow-lg flex items-center gap-1"
          >
      ...
      <div class="p-4">
        ...
        <div class="flex items-start justify-between gap-2 mb-2">
          <h3
            class="text-sm font-semibold text-gray-900 leading-tight flex-1 truncate"
          >
            {{ produit.nom }}
          </h3>
        </div>
        ...
        <!-- Catégorie et ville -->
        <div
          v-if="produit.category?.nom"
          class="mt-1 flex justify-between w-full"
        >
          <span
            class="inline-block bg-gray-200 text-gray-700 text-sm py-1 rounded-full px-1"
          >
            {{ produit.category.nom }}
          </span>
        </div>
        <div
          class="flex items-center gap-1 text-end text-gray-500"
          v-if="produit.ville"
        >
          <i class="fas fa-map-marker-alt text-sm"></i>
          <span class="text-sm">{{ produit.ville }}</span>
        </div>
      </div>
    </div>
