...
<template>
  <div class="shadow-black shadow-2xl z-240 bg-primary border-t border-primary-light">
      ...
      <nav class="lg:hidden shadow-gray-900 shadow-xl z-240">
        <div class="flex justify-around items-center py-1">
          <RouterLink
            ...
            class="relative flex flex-col items-center justify-center w-15 hover:text-secondary transition-colors duration-300 group featured-item"
            :class="[
              link.featured ? '' : '',
              $route.path === link.to ? 'text-secondary font-bold' : 'text-white/70',
            ]"
            active-class="text-secondary"
            @click="
              link.badgeKey
                ? badgeStore.markAsRead(link.badgeKey as any)
                : null
              "
            >
              <div class="relative text-center">
                <!-- Icône principale -->
                <i
                  class="fas transition-all duration-300 transform group-hover:scale-110"
                  :class="[
                    link.icon,
                    !authStore.user && link.to === '/home'
                      ? 'animate-swing'
                      : '',
                    link.featured ? 'text-xl sm:text-sm' : 'text-sm sm:text-xl',
                  ]"
                >
                </i>
                ...
                <span
                  v-if="getBadgeForRoute(link.to) > 0"
                  class="cart-badge absolute -top-1 -right-2 bg-secondary text-sm rounded-full h-5 w-5 flex items-center justify-center font-bold"
                  :class="{
                    'animate-scale': getAnimateStateForRoute(link.to),
                    'featured-badge': link.featured,
                  }"
                  :aria-label="
                    getBadgeDescription(link.to, getBadgeForRoute(link.to))
                  "
                >
                  {{
                    getBadgeForRoute(link.to) > 99
                      ? "99+"
                      : getBadgeForRoute(link.to)
                  }}
                </span>
              </div>
              ...
              <!-- Label -->
              <span
                class="text-[10px] mt-1 font-medium max-w-full px-1 text-center transition-all duration-300"
                :class="
                  link.featured
                    ? 'font-bold transform group-hover:scale-105'
                    : ''
                "
              >
                {{ link.label }}
              ...
              <div
                v-if="link.featured"
                class="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"
              ></div>
            </RouterLink>
          </div>
        </nav>
