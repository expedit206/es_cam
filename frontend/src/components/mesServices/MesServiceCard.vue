<template>
  <div
    class="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-all duration-300 overflow-hidden group hover:-translate-y-1"
    v-if="service"
  >
    ...
    <div class="relative aspect-[4/3] overflow-hidden" v-if="service">
      ...
      <div class="absolute top-2 left-2">
        <span
          v-if="service.disponibilite === 'disponible'"
      ...
      <div class="absolute top-2 right-2">
        <div class="relative">
          ...
          >
            <button
              @click="handleEdit"
              class="w-full text-left px-3 py-1 text-sm hover:bg-green-50 transition-colors flex items-center gap-2"
            >
            ...
            <button
              ...
              class="w-full text-left px-3 py-1 text-sm hover:bg-blue-50 transition-colors flex items-center gap-2"
            >
              <i
                :class="
            ...
            <button
              @click="handleDelete"
              class="w-full text-left px-3 py-1 text-sm hover:bg-red-50 transition-colors flex items-center gap-2"
            >
    ...
    <div class="p-3">
      <!-- Titre -->
      <h3
        class="font-semibold text-gray-900 text-sm leading-tight mb-2 line-clamp-1"
      >
        {{ service.titre }}
      </h3>
      ...
      <!-- Compétences -->
      <div
        v-if="service.competences && service.competences.length > 0"
        class="mb-2"
      >
        <div class="flex flex-wrap gap-1">
          <span
            v-for="(competence, index) in service.competences.slice(0, 2)"
            :key="index"
          ...
          <span
            v-if="service.competences.length > 2"
      ...
      <div class="text-[10px] text-gray-500 flex items-center gap-1 truncate">
        ...
        <span class="truncate"
          >{{ service.ville
          }}{{ service.localisation ? ", " + service.localisation : "" }}</span
        >
      </div>
    </div>
  </div>
