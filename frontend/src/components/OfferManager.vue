<template>
  ...
  >
    <div
      class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[100vh] overflow-hidden pb-24"
    >
      <!-- En-tête simple -->
      ...
      <div class="flex flex-col h-[calc(90vh-120px)] overflow-y-auto mb-24">
                      ...
                      <div class="flex items-center gap-3 mb-2">
                        ...
                        <div class="flex-1">
                          <div class="flex items-center gap-2 mb-1">
                            ...
                            <span
                              class="font-semibold text-[var(--espace-vert)]"
                            >
        ...
        <div
          v-if="isEditing"
          class="border-t border-gray-200 bg-white p-6 overflow-y-auto h-full pb-8"
        >
              ...
              <form @submit.prevent="saveOffer" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      v-model.number="currentOffer.nombre_jetons"
                      type="number"
                      min="1"
                  ...
                  <div>
                    ...
                    <input
                      v-model.number="currentOffer.prix_unitaire"
                      type="number"
                      min="1"
                ...
                <div>
                  ...
                  <select
                    v-model="currentOffer.wallet_id"
                    required
                    class="w-full rounded-lg border border-gray-300 px-4 py-3 focus:outline-none focus:border-[var(--espace-vert)] focus:ring-1 focus:ring-[var(--espace-vert)] transition-colors"
                  >
...
const averagePrice = computed(() => {
  ...
  const total = offers.value.reduce(
    (sum, offer) => parseFloat(sum) + parseFloat(offer.prix_unitaire),
    0,
  );
...
const saveOffer = async () => {
  ...
  try {
    if (currentOffer.value.id) {
      // Modification
      await apiClient.put(
        `/jeton_market/updateOffer/${currentOffer.value.id}`,
        currentOffer.value,
      );
