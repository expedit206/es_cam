<template>
  ...
  >
    <div
      class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-modal-in"
    >
      <!-- En-tête avec dégradé -->
      <div
        class="bg-gradient-to-r from-[var(--espace-vert)] to-green-600 p-6 text-white"
      >
      ...
      <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
        ...
        >
          <div
            class="flex items-center justify-center gap-2 text-sm text-gray-600 mb-1"
          >
            <i class="fas fa-tag text-green-600"></i>
            <span>{{ t("current_rate") }}</span>
          </div>
          <div class="text-2xl font-bold text-[var(--espace-vert)]">
            {{ t("token_price_display") }}
          </div>
        </div>
              ...
              <input
                ...
                :class="[
                  ...
                  emailError
                    ...
                    : 'border-gray-200 focus:border-[var(--espace-vert)] focus:ring-[var(--espace-vert)]/20',
                ]"
              />
              <div
        ...
        <!-- Récapitulatif -->
        <div
          class="bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl p-4 border border-gray-200"
        >
          <div class="flex justify-between items-center mb-2">
            <span class="text-sm text-gray-600">{{ t("token_quantity") }}</span>
            <span class="font-semibold text-gray-900"
              >{{ jetonQuantity }} {{ t("tokens") }}</span
            >
          </div>
          <div class="flex justify-between items-center">
            <span class="text-sm text-gray-600">{{ t("total_amount") }}</span>
            <span class="text-xl font-bold text-[var(--espace-vert)]"
              >{{ montant.toLocaleString() }} FCFA</span
            >
          </div>
        </div>
    ...
    if (paymentData.value?.redirect_url) {
      ...
      if (paymentData.value.transaction_id) {
        localStorage.setItem(
          "pending_transaction",
          paymentData.value.transaction_id,
        );
      }
