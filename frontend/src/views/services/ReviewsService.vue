<template>
  <div
    class="bg-slate-100 overflow-y-auto h-[15rem] rounded-xl shadow-sm border border-gray-200 p-3 py-1"
  >
    <!-- Résumé des notes -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-2 mb-2">
      <!-- Note moyenne -->
      <div class="text-center">
        <div class="text-md text-sm sm:text-lg font-bold text-gray-900">
          {{ reviewsStats.average }} ({{ reviewsStats.total }})
        </div>
    ...
    <div
      v-if="userStore.user?.id && !userHasReviewed"
      class="mb-2 px-4 rounded-lg"
    >
      <ReviewForm
      ...
      >
        <div class="flex items-start space-x-3">
          ...
          <div class="flex-1">
            ...
            <p class="text-gray-700 mb-1 text-sm">{{ review.comment }}</p>
...
const fetchReviews = async (page = 1) => {
  try {
    ...
    const response = await apiClient.get(
      `/services/${props.serviceId}/getReviews`,
      {
        params: { page },
      },
    );
