    ...
    >
          ...
          >
            <td class="px-6 py-4">
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center text-gray-500 font-bold overflow-hidden border border-gray-100"
                >
                  <img
                    v-if="user.photo"
                    :src="user.photo"
      ...
      >
        <span class="text-sm text-gray-500"
          >Page {{ adminStore.pagination.users.current }} sur
          {{ adminStore.pagination.users.last }}</span
        >
        <div class="flex gap-2">
          ...
          <button
            @click="nextPage"
            :disabled="
              adminStore.pagination.users.current ===
              adminStore.pagination.users.last
            "
            class="px-4 py-2 bg-white rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-50 transition-all font-medium text-sm"
          >
            Suivant
