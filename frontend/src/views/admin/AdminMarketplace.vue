    ...
    >
      <table class="w-full">
        <thead>
          <tr class="bg-gray-50 text-left">
            ...
            <th
              class="px-6 py-4 text-sm font-bold text-gray-500 uppercase text-right"
            >
              Actions
            </th>
          </tr>
        ...
        <tbody class="divide-y divide-gray-100">
          <template v-if="activeTab === 'products'">
            <tr
              v-for="item in adminStore.products"
              :key="item.id"
            ...
            >
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <div
                    class="w-12 h-12 rounded-xl bg-gray-100 flex-shrink-0 overflow-hidden"
                  >
                    <img
                    ...
                    <i
                      v-else
                      class="fas fa-image text-gray-300 m-auto flex h-full"
                    ></i>
                  </div>
                  <div>
                    <div class="font-bold text-gray-800 line-clamp-1">
                      {{ item.nom }}
                    </div>
                    <div class="text-sm text-[var(--espace-vert)] font-bold">
                      {{ item.prix }} XAF
                    </div>
                  </div>
                </div>
              ...
              <td class="px-6 py-4 text-right">
                <div class="flex justify-end gap-2">
                  ...
                  >
                    <i
                      class="fas"
                      :class="item.est_actif ? 'fa-eye-slash' : 'fa-eye'"
                    ></i>
                  </button>
                  <button
          ...
          <template v-else>
            <tr
              v-for="item in adminStore.services"
              :key="item.id"
              class="hover:bg-gray-50/50 transition-colors"
            >
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  ...
                  >
                    ...
                    <i
                      v-else
                      class="fas fa-concierge-bell text-gray-300 m-auto flex h-full items-center justify-center"
                    ></i>
                  </div>
                  <div>
                    <div class="font-bold text-gray-800 line-clamp-1">
                    ...
                    <div class="text-sm text-gray-400">{{ item.ville }}</div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4">
      ...
      >
        <span class="text-sm text-gray-500">
          ...
          {{
            activeTab === "products"
              ? adminStore.pagination.products.current
              : adminStore.pagination.services.current
          }}
          sur
          {{
            activeTab === "products"
              ? adminStore.pagination.products.last
              : adminStore.pagination.services.last
          }}
        </span>
...
.tab-btn.active {
  @apply bg-primary text-white shadow-lg shadow-green-500/20;
}
</style>
