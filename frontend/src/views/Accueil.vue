<template>
  ...
  <div class="h-full pb- lg:pb-0 bg-background">
    <header
      class="relative bg-gradient-to-br from-primary to-primary-dark text-white overflow-hidden  shadow-2xl"
    >
      ...
      <div class="container mx-auto px-4 py-2 lg:py-4 relative z-10">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 items-center">
          <div class="text-center lg:text-left space-y-2 animate-fade-in-left">
            ...
            >
              ...
              <span
                class="text-xs sm:text-sm !text-white font-semibold tracking-wide uppercase font-heading"
                >1ère Marketplace locale au Cameroun 🇨🇲</span
              >
            </div>

            <h1
              class="text-sm sm:text-2xl lg:text-3xl font-extrabold tracking-tight leading-[1.1] !text-gray-200 font-heading"
            >
              Achetez, vendez et négociez  <br />
              <span
                class="text-base sm:text-3xl text-white bg-clip-text bg-gradient-to-r from-secondary to-yellow-200"
              >
                près de chez vous
              </span>
            </h1>
            ...
            <div class="space-y-4">
              ...
              >
                ...
                <input
                  ...
                  class="flex-1 w-full bg-transparent border-none focus:ring-0 text-foreground placeholder-gray-400 font-medium px-3 py-2 font-body text-sm"
          ...
          <!-- SECTION AVEC ANIMATION PARALLAX MOUSE-OVER -->
          <!-- SECTION AVEC ANIMATION PARALLAX MOUSE-OVER (Desktop Only) -->
          <div class="hidden lg:block relative animate-fade-in-right">
            <div
              v-if="heroDisplayProducts.length >= 3"
              class="relative z-20 grid grid-cols-2 gap-3 p-2 perspective-1000"
            ...
            >
              ...
              <div
                class="col-span-2 relative overflow-hidden rounded-2xl shadow-xl border-2 border-white/20 transition-transform duration-300 ease-out cursor-pointer group"
                :style="{
                ...
                @click="viewProduct(heroDisplayProducts[0])"
              >
                <img
                  :src="heroDisplayProducts[0].photos[0]"
                  :alt="heroDisplayProducts[0].nom"
                  class="w-full h-[240px] object-cover transition-transform duration-700"
                  :style="{
                    transform:
                ...
                <!-- Tag au survol -->
                <div
                  class="absolute top-3 right-3 bg-secondary text-primary-dark px-3 py-1 rounded-full text-xs font-bold shadow-lg transform transition-all duration-300 font-heading"
                >
                  {{ heroDisplayProducts[0].prix }} FCFA
                </div>
                <div class="absolute bottom-3 left-3 text-white">
                  <p class="font-bold text-sm line-clamp-1">
                    {{ heroDisplayProducts[0].nom }}
                  </p>
                </div>
              </div>
              ...
              <div
                class="relative overflow-hidden rounded-xl shadow-lg border-2 border-white/10 transition-all duration-300 ease-out cursor-pointer group"
                :style="{
                ...
                @click="viewProduct(heroDisplayProducts[1])"
              >
                <img
                  :src="heroDisplayProducts[1].photos[0]"
                  :alt="heroDisplayProducts[1].nom"
                  class="w-full h-[160px] object-cover transition-transform duration-500"
                  :style="{
                    transform:
                ...
                <!-- Overlay texte -->
                <div
                  class="absolute bottom-3 left-3 text-white transition-all duration-300"
                >
                  <div class="text-xs font-bold font-heading !text-white line-clamp-1">
                    {{ heroDisplayProducts[1].nom }}
                  </div>
                  <div class="text-[10px] font-body text-gray-200">
                    {{ heroDisplayProducts[1].prix }} FCFA
                  </div>
                </div>
              </div>
              ...
              <div
                ...
                @click="viewProduct(heroDisplayProducts[2])"
              >
                <img
                  :src="heroDisplayProducts[2].photos[0]"
                  :alt="heroDisplayProducts[2].nom"
                  class="w-full h-[160px] object-cover transition-transform duration-500"
                  :style="{
                    transform:
                ...
                <div
                  class="absolute bottom-3 left-3 text-white transition-all duration-300"
                >
                  <div class="text-xs font-bold !text-white  font-heading line-clamp-1">
                    {{ heroDisplayProducts[2].nom }}
                  </div>
                  <div class="text-[10px] font-body text-gray-200">
                    {{ heroDisplayProducts[2].prix }} FCFA
                  </div>
                </div>
              </div>
        ...
        <!-- MOBILE HERO IMAGES (Carousel) -->
        <div
          class="lg:hidden mt-8 -mx-4 px-4 overflow-x-auto hide-scrollbar pb-4"
        >
          <div class="flex gap-4" v-if="heroDisplayProducts.length > 0">
            <div
              v-for="(product, idx) in heroDisplayProducts"
              :key="'mobile-hero-' + product.id"
              class="relative min-w-[260px] h-[200px] rounded-2xl overflow-hidden shadow-lg border border-white/20 flex-shrink-0"
            ...
            >
              ...
              <div class="absolute bottom-3 left-3 right-3 text-white">
                <p class="font-bold text-sm truncate">{{ product.nom }}</p>
                <p class="text-xs text-yellow-300 font-bold">
                  {{ product.prix }} FCFA
                </p>
              </div>
              <div
                class="absolute top-2 right-2 bg-secondary text-primary-dark text-[10px] font-bold px-2 py-0.5 rounded-full"
              >
                A la une
              </div>
            </div>
          </div>
          <!-- Empty state for mobile if no products -->
          <div
            v-else
            class="h-40 bg-white/10 rounded-2xl flex items-center justify-center border border-white/10 border-dashed text-white/50 text-sm"
          >
            Bientôt des produits à la une...
          </div>
        </div>
      </div>
    </header>
...
const heroDisplayProducts = computed(() => {
  // Select 3 random products with photos or just the first 3
  const withPhotos = marketplaceStore.products.filter(
    (p) => p.photos && p.photos.length > 0,
  );
  if (withPhotos.length >= 3) {
    return withPhotos.slice(0, 3);
  }
  return marketplaceStore.products.slice(0, 3);
});
