  ...
  <div class="min-h-screen bg-gray-50 pb-12">
    ...
    <div class="bg-white shadow-sm mb-6">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <p class="mt-2 text-gray-600">Retrouvez tous vos articles préférés</p>
      </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div v-if="loading" class="flex justify-center items-center py-20">
        <div
          class="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[var(--espace-vert)]"
        ></div>
      </div>
      ...
      <div v-else>
        ...
        <div v-if="activeTab === 'products'" class="animate-fade-in-up">
          ...
          >
            ...
            <router-link
              to="/market-place"
              class="inline-flex items-center px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-[var(--espace-vert)] hover:bg-green-800 transition-colors"
            >
          ...
          >
            ...
            >
              <MesProductCard :produit="product" />
              <button
                @click.prevent="removeFavorite(product.id, 'product')"
                class="absolute bottom-6 right-1 z-20 p-2 text-red-500 hover:scale-110 transition-transform"
              >
        ...
        <div v-if="activeTab === 'services'" class="animate-fade-in-up">
          ...
          >
            ...
            >
              <!-- Delete Button -->
              <button
                @click.prevent="removeFavorite(service.id, 'service')"
              ...
              <router-link
                ...
                class="relative pt-[60%] overflow-hidden bg-gray-100 flex-shrink-0"
              >
                <img
                ...
                >
                  <span
                    class="text-white text-sm font-bold px-2 py-1 bg-[var(--espace-or)] rounded text-[var(--espace-vert)]"
                  >
              ...
              <div class="p-4 flex flex-col flex-1">
                <router-link :to="'/services/' + service.id" class="block">
                  <h3
                    class="text-gray-900 font-semibold text-lg mb-2 line-clamp-1 group-hover:text-[var(--espace-vert)] transition-colors"
                  >
                    {{ service.titre }}
                  </h3>
                </router-link>
                ...
                >
                  <div class="flex items-center gap-2">
                    ...
                    >
                      <img
                        v-if="service.user?.photo"
                        :src="getStorageUrl(service.user.photo)"
                        class="w-full h-full object-cover"
                      />
                      <div
                        v-else
                        class="w-full h-full flex items-center justify-center text-gray-400 text-sm font-bold"
                      >
                        {{ service.user?.nom?.charAt(0) || "U" }}
                      </div>
                    </div>
                    <span
                      class="text-sm font-medium text-gray-700 truncate max-w-[100px]"
                      >{{ service.user?.nom || "Prestataire" }}</span
                    >
                  </div>
...
onMounted(() => {
  if (checkAuth("Connectez-vous pour voir vos favoris")) {
    fetchFavorites();
  }
});
</script>
