<template>
  <div class="min-h-screen bg-white text-gray-900">
    ...
    <!-- Hero section professionnelle -->
        <div
          class="relative bg-gradient-to-br from-primary/5 via-white to-primary/5 overflow-hidden border-b border-gray-100">
          ...
          <div class="max-w-7xl mx-auto px-4 py-12 md:py-16 relative">
            <div class="max-w-3xl">
              ...
              <div class="inline-flex items-center gap-2 bg-primary/10 px-3 py-1 rounded-full mb-4">
                ...
                <span class="text-xs font-semibold uppercase tracking-wider text-primary">Le blog</span>
              </div>

              <!-- Titre principal -->
              <h1 class="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 leading-tight">
                Explorez, apprenez et <span class="text-primary">vendez mieux</span>
              </h1>

              <!-- Description enrichie -->
              <p class="text-base md:text-lg text-gray-600 mt-4 max-w-2xl leading-relaxed">
                Actualités, conseils d'experts et guides pratiques pour booster votre activité sur Espace Cameroun.
              </p>
              ...
              <div class="mt-6">
                <button
                  ...
                  class="inline-flex items-center gap-2 border border-gray-300 bg-white px-5 py-2 rounded-full text-sm font-medium text-gray-700 hover:bg-gray-50 transition shadow-sm">
    ...
    <div class="max-w-7xl mx-auto px-4 py-6 md:py-8">
      <!-- Loading State -->
      <div
        v-if="blogStore.isLoading && !blogStore.posts.length"
        class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"
      >
        <div
          v-for="i in 6"
          :key="i"
          class="bg-white border border-gray-100 rounded-lg p-3 h-64 animate-pulse"
        >
      ...
      <div v-else-if="blogStore.posts.length > 0">
        ...
        <section v-if="featuredPost" class="mb-6">
          <article
            class="bg-white border border-gray-100 rounded-lg overflow-hidden hover:border-gray-200 transition-colors"
          >
            <div class="grid md:grid-cols-2">
              <!-- Image -->
              <router-link
                :to="`/blog/${featuredPost.slug}`"
                class="block h-48 md:h-auto bg-gray-100"
              >
                <img
              ...
              <div class="p-5">
                ...
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div
                      class="w-6 h-6 rounded-full bg-gray-200 overflow-hidden"
                    >
                      <img
                        v-if="featuredPost.author.photo"
                        :src="getImageUrl(featuredPost.author.photo)"
                        class="w-full h-full object-cover"
                      />
                      <div
                        v-else
                        class="w-full h-full flex items-center justify-center bg-gray-700 text-white text-[10px] font-bold"
                      >
                        {{ featuredPost.author.nom.charAt(0) }}
                      </div>
                    </div>
                    <span class="text-sm font-medium text-gray-700">{{
                      featuredPost.author.nom
                    }}</span>
                  </div>
                  <router-link
                    :to="`/blog/${featuredPost.slug}`"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          >
            ...
            <router-link
              :to="`/blog/${post.slug}`"
              class="block h-36 bg-gray-100 relative overflow-hidden"
            >
              <img
                v-if="post.image"
              ...
              <span
                v-if="post.video"
                class="absolute bottom-1 right-1 w-5 h-5 bg-white rounded-full flex items-center justify-center shadow"
              >
            ...
            <div class="p-3">
              <div
                class="flex items-center gap-2 text-[10px] text-gray-500 mb-1"
              >
              ...
              <h3
                class="text-sm font-bold text-gray-900 mb-1 line-clamp-2 leading-snug"
              >
                <router-link
                  :to="`/blog/${post.slug}`"
                  class="hover:text-primary"
              ...
              <div class="flex items-center justify-between mt-1">
                <div class="flex items-center gap-1">
                  <div class="w-4 h-4 rounded-full bg-gray-200 overflow-hidden">
                    ...
                    <div
                      v-else
                      class="w-full h-full flex items-center justify-center bg-gray-600 text-white text-[6px] font-bold"
                    >
                      {{ post.author.nom.charAt(0) }}
                    </div>
                  </div>
                  <span
                    class="text-[10px] text-gray-600 truncate max-w-[80px]"
                    >{{ post.author.nom }}</span
                  >
                </div>
                <span
                  class="text-[10px] text-primary font-medium transition-opacity"
                >
                  Lire →
                </span>
              </div>
        ...
        <!-- Pagination Compact -->
        <div
          v-if="blogStore.pagination.lastPage > 1"
          class="mt-8 flex justify-center"
        >
          <div
            class="inline-flex items-center gap-1 p-1 bg-gray-50 rounded-lg border border-gray-100"
          >
            <button
              @click="changePage(blogStore.pagination.currentPage - 1)"
              :disabled="blogStore.pagination.currentPage === 1"
              class="w-7 h-7 flex items-center justify-center rounded text-sm text-gray-600 hover:bg-white disabled:opacity-30"
            >
              <i class="fas fa-chevron-left"></i>
            </button>

            <span class="text-sm text-gray-600 px-2">
              {{ blogStore.pagination.currentPage }} /
              {{ blogStore.pagination.lastPage }}
            </span>
    ...
    <!-- Modal de recherche -->
    <SearchBlogModal :isOpen="isSearchOpen" @close="isSearchOpen = false" />
  </div>
</template>
...
import SearchBlogModal from "./SearchBlogModal.vue";
