                ...
                <form @submit.prevent="submit" class="space-y-4 sm:space-y-6">
                    ...
                    <div>
                        ...
                        <div class="border-2 border-dashed border-gray-300 rounded-xl sm:rounded-2xl p-3 sm:p-4">
                            ...
                            <input ref="fileInput" type="file" multiple @change="handleFileChange" class="hidden"
                                accept="image/*" />
                            <button type="button" @click="fileInput?.click()" :disabled="imagePreviews.length >= 5"
                                :class="[
                                ...
                                <span class="text-xs text-gray-500">({{ imagePreviews.length }}/5)</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        <div class="relative">
                            ...
                            <input v-model="categorySearch" type="text" :placeholder="t('search_category')"
                            ...
                            <ul v-if="showCategoryDropdown && filteredCategories.length"
                        ...
                        <div>
                            ...
                            <input v-model="form.titre" type="text" required maxlength="255"
                                class="w-full p-2.5 sm:p-3 border border-gray-300 rounded-lg sm:rounded-xl focus:border-[var(--espace-vert)] focus:ring-2 focus:ring-green-200 outline-none transition-all text-sm"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        ...
                        <div>
                            ...
                            <input v-model="form.localisation" type="text" required
                                class="w-full p-2.5 sm:p-3 border border-gray-300 rounded-lg sm:rounded-xl focus:border-[var(--espace-vert)] focus:ring-2 focus:ring-green-200 outline-none transition-all text-sm"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        <div>
                            ...
                            <input v-model.number="form.annees_experience" type="number" min="0" max="50"
                                class="w-full p-2.5 sm:p-3 border border-gray-300 rounded-lg sm:rounded-xl focus:border-[var(--espace-vert)] focus:ring-2 focus:ring-green-200 outline-none transition-all text-sm"
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-2 mb-2">
                                <span v-for="(skill, index) in form.competences" :key="index"
                                    class="bg-green-100 text-green-700 text-xs px-3 py-1 rounded-full flex items-center gap-1">
                                    {{ skill }}
                                    <button type="button" @click="removeSkill(index)" class="hover:text-green-900">
                                        <i class="fas fa-times text-[10px]"></i>
                                    </button>
                                </span>
                            </div>
                            <div class="flex gap-2">
                                <input v-model="newSkill" type="text" @keydown.enter.prevent="addSkill"
                                    class="flex-1 p-2.5 sm:p-3 border border-gray-300 rounded-lg sm:rounded-xl focus:border-[var(--espace-vert)] focus:ring-2 focus:ring-green-200 outline-none transition-all text-sm"
                    ...
                    <div>
                        ...
                        <textarea v-model="form.description" rows="4" required
                            class="w-full p-2.5 sm:p-3 border border-gray-300 rounded-lg sm:rounded-xl focus:border-[var(--espace-vert)] focus:ring-2 focus:ring-green-200 outline-none transition-all text-sm"
                            :placeholder="t('service_description_placeholder')"></textarea>
                    ...
                    <div class="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 bg-gray-50 rounded-lg sm:rounded-xl">
                        <input id="disponible" type="checkbox" v-model="form.disponibilite" true-value="disponible"
...
const addSkill = () => {
    if (newSkill.value.trim() && !form.value.competences.includes(newSkill.value.trim())) {
...
const submit = async () => {
    ...
    // Validation
    if (!form.value.titre.trim() || !form.value.description.trim() || !form.value.category_id) {
    ...
    try {
        ...
        if (form.value.annees_experience) {
            formData.append('annees_experience', form.value.annees_experience.toString())
        }
...
const res = ref()
        if (isEditMode.value && props.service) {
          res.value =  await apiClient.post(`/services/${props.service.id}`, formData, {
...
watch(() => props.visible, (newVal) => {
    if (newVal && props.service) {
        ...
        nextTick(() => {
            ...
            oldImages.value = props.service?.images ? [...props.service.images] : []
            newImages.value = []
            categorySearch.value = props.categories.find(c => c.id.toString() === props.service?.category_id?.toString())?.nom || ''
        })
    }
})
...
watch(() => props.service, (newService) => {
    if (newService) {
        ...
        newImages.value = []
        categorySearch.value = props.categories.find(c => c.id.toString() === newService.category_id.toString())?.nom || ''
    }
})
</script>