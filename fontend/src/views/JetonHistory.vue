<template>
    ...
    <StructuredData :type="transactionStore.structuredData.type" :data="transactionStore.structuredData.data" />
        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
                <div class="bg-white rounded-2xl shadow-sm border p-2 text-center">
                    <div class="text-sm font-bold text-[var(--espace-vert)] mb-1">
                ...
                <div class="bg-white rounded-2xl shadow-sm border p-2 text-center">
                    <div class="text-sm font-bold text-[var(--espace-or)] mb-1">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center mt-5 mb-4">
                <router-link to="/acheter-jetons"
                    class="bg-[var(--espace-vert)] text-white px-6 py-3 rounded-xl font-semibold hover:bg-green-700 transition-all flex items-center justify-center gap-2">
                ...
                <div class="flex flex-wrap gap-2 items-center">
                    ...
                    <div class="border-b-1 pb-2 w-full">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button v-for="type in transactionStore.typeFilters" :key="type.value"
                                @click="transactionStore.setActiveTypeFilter(type.value)" :class="[
                    ...
                    <div class="border-b-1 pb-2 w-full">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button v-for="filter in transactionStore.statusFilters" :key="filter.value"
                                @click="transactionStore.setActiveStatusFilter(filter.value)" :class="[
                                    'px-2 py-1 rounded-lg text-xs font-medium transition-all',
                    ...
                    <div class="">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button v-for="role in transactionStore.roleFilters" :key="role.value"
                                @click="transactionStore.setActiveRoleFilter(role.value)" :class="[
                                    'px-2 py-1 rounded-lg text-xs font-medium transition-all',
            ...
            <div v-if="transactionStore.filteredTransactions.length > 0" class="space-y-3 overflow-y-scroll h-[90vh]">
                <div v-for="transaction in transactionStore.filteredTransactions" :key="transaction.id"
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-3">
                            <div :class="[
                                'w-8 h-8 rounded-xl flex items-center justify-center text-white text-xs',
                                transactionStore.getTransactionTypeIcon(transaction.type).bgColor
                            ]">
                                <i :class="transactionStore.getTransactionTypeIcon(transaction.type).icon"
                                    class="text-xs"></i>
                            </div>
                            <div>
                                <h3 class="text-xs md:text-lg font-semibold text-gray-900">
                        ...
                        <div class="text-right">
                            <div class="text-sm font-bold" :class="transactionStore.getAmountColor(transaction)">
                                {{ transactionStore.formatPrice(transaction.montant_total) }} FCFA
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 mb-4">
                        <!-- Prix unitaire -->
                        <div class="bg-gray-50 rounded-lg p-1 flex justify-between items-center text-xs">
                            <div class="text-gray-600">{{ t('unit_price') }}</div>
                            <div class="font-semibold text-gray-900">
                                {{ transactionStore.formatPrice(transaction.prix_unitaire) }} FCFA
                            </div>
                        </div>

                        <!-- Montant net (pour vendeur) -->
                        <div v-if="transactionStore.isVendeur(transaction)"
                            class="bg-green-50 rounded-lg p-1 flex justify-between items-center text-xs">
                            <div class="text-green-700">{{ t('net_amount') }}</div>
                            <div class="font-semibold text-green-900">
                                {{ transactionStore.formatPrice(transaction.montant_net_vendeur || 0) }} FCFA
                            </div>
                        </div>

                        <!-- Commission (pour vendeur) -->
                        <div v-if="transactionStore.isVendeur(transaction)"
                            class="bg-orange-50 rounded-lg p-1 flex justify-between items-center text-xs">
                            <div class="text-orange-700">{{ t('commission') }}</div>
                            <div class="font-semibold text-orange-900">
                                -{{ transactionStore.formatPrice(transaction.commission_plateforme || 0) }} FCFA
                            </div>
                        </div>

                        <!-- Référence -->
                        <div class="bg-gray-50 rounded-lg p-1 flex justify-between items-center gap-2">
                            <div class="text-xs text-gray-600">{{ t('reference') }}</div>
                    ...
                    <div class="flex flex-wrap items-center gap-4 text-xs mb-3">
                        ...
                        <span :class="[
                            ...
                            <i :class="transactionStore.getStatusIcon(transaction.statut)"></i>
                            {{ transactionStore.getStatusText(transaction.statut) }}
                        </span>

                        <!-- Partie adverse -->
                        <span v-if="transactionStore.isAcheteur(transaction) && transaction.vendeur"
                            class="px-2 py-1 bg-blue-100 text-blue-700 rounded-full border border-blue-200">
                            <i class="fas fa-user mr-1"></i>
                            {{ t('seller') }}: {{ transaction.vendeur.nom }}
                        </span>

                        <span v-if="transactionStore.isVendeur(transaction) && transaction.acheteur"
                    ...
                    <div class="flex justify-end gap-2 mt-2 pt-2 border-t border-gray-200">
                        <!-- Vérification statut -->
                        <button v-if="transaction.statut === 'pending'" @click="handleCheckStatus(transaction)"
                            :disabled="transaction.checkingStatus"
                            class="flex items-center gap-2 text-[var(--espace-vert)] border-[var(--espace-vert)] border-2 px-4 py-2 rounded-lg hover:bg-blue-300 transition-all text-xs disabled:opacity-50 disabled:cursor-not-allowed">
                            <i :class="[
                                'fas',
                                transaction.checkingStatus ? 'fa-spinner fa-spin' : 'fa-sync-alt'
                            ]"></i>
                        ...
                        <button
                            v-if="transactionStore.isAcheteur(transaction) && transaction.type === 'platform' && transaction.statut === 'complete'"
                            @click="transactionStore.redoPlatformPurchase(transaction)"
                            class="flex items-center gap-2 bg-[var(--espace-vert)] text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-all text-sm">
            ...
            <!-- Pagination -->
            <div v-if="transactionStore.filteredTransactions.length > 0" class="flex justify-center mt-8">
                <div class="bg-white rounded-2xl shadow-sm border px-6 py-3">
                    <div class="flex items-center gap-4 text-sm">
                        <button @click="transactionStore.loadPreviousPage"
                            :disabled="transactionStore.currentPage === 1" :class="[
                        ...
                        <button @click="transactionStore.loadNextPage"
                            :disabled="transactionStore.currentPage === transactionStore.totalPages" :class="[
                                'px-4 py-2 rounded-lg transition-all',
