<template>
    ...
    <div class="min-h-screen bg-gray-50">
                ...
                <div v-if="showSearchBar" class="mt-3  animate-slide-down">
                    <div class="relative">
                        ...
                        <input v-model="filters.searchQuery" type="text" :placeholder="getSearchPlaceholder()"
                            ...
                            @input="handleSearchInput" ref="searchInput">
                        <button v-if="filters.searchQuery" @click="setSearchQuery('')"
                            class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600">
        ...
        <div class="bg-white sticky  z-30 ">
            <div class="max-w-7xl mx-auto">


                <div class="flex overflow-x-auto hide-scrollbar bg-gray-100">
                    <!-- {{ getFilteredCategories() }} -->
                    <button v-for="category in getFilteredCategories()" :key="category.id"
                        @click="setActiveCategory(category.id)" :class="[
                            ...
                            filters.activeCategory === category.id
                                ? 'border-[var(--espace-vert)] text-[var(--espace-vert)] bg-green-50'
        ...
        <div class="max-w-7xl mx-auto px-4 py-6 bg-slate-100">
            ...
            <div v-else-if="filters.activeCategory === 'all' && !filters.searchQuery" class="space-y-8">
                ...
                <section>
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                        <template v-for="item in allItems" :key="`item-${item.id}`">
                            <MesProductCard v-if="filters.contentType === 'product'" :produit="item" view-mode="grid"
                                @click="viewProduct(item)" />
                            <ServiceCard v-else :service="item" view-mode="grid" @click="viewService(item)" />
                        </template>
                    </div>

                    <!-- Élément sentinelle pour le scroll infini -->
                    <div ref="infiniteScrollTrigger" class="mt-4 pb-8">
                        <div v-if="isLoadingMore" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                            <ProductCardSkeleton v-for="i in 12" :key="i" />
                        </div>
                        <div v-else-if="!hasMoreItems && allItems.length > 0" class="flex justify-center">
            ...
            <div v-else>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <p class="text-gray-600 text-sm">
                                ...
                                <span v-if="filters.searchQuery" class="text-[var(--espace-vert)]">
                                    pour "{{ filters.searchQuery }}"
                                </span>
                                <span v-else-if="filters.activeCategory !== 'all'" class="text-[var(--espace-vert)]">
                ...
                <div :class="[
                    ...
                    <template v-for="item in allItems" :key="`${item.id}`">
                        <MesProductCard v-if="filters.contentType === 'product'" :produit="item"
                            :view-mode="filters.viewMode" @click="viewProduct(item)" />
                        <ServiceCard v-else :service="item" :view-mode="filters.viewMode" @click="viewService(item)" />
                    </template>
                </div>
...
const hasMoreItems = computed(() => marketplaceStore.pagination.currentPage < marketplaceStore.pagination.lastPage);
...
const allItems = computed(() => {
    return filters.contentType === 'product' ? marketplaceStore.products : marketplaceStore.services;
});
...
const getSectionTitle = () => {
    ...
    if (filters.activeCategory !== 'all') {
        return categories.value.find(c => c.id === filters.activeCategory)?.name || 'Catégorie';
    }
...
watch(() => filters.contentType, async (newType) => {
    if (newType === 'product' && marketplaceStore.products.length === 0) {
        await marketplaceStore.fetchProducts();
    } else if (newType === 'service' && marketplaceStore.services.length === 0) {
...
watch([() => filters.activeCategory, () => filters.contentType, () => filters.searchQuery], () => {
    nextTick(() => {
        setupIntersectionObserver();
    });
});
...
watch(() => route.query.search, (newSearch) => {
    if (newSearch !== undefined && newSearch !== marketplaceStore.filters.searchQuery) {
