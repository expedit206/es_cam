    ...
    <div class="bg-slate-100 overflow-y-auto h-[15rem] rounded-xl shadow-sm border border-gray-200 p-3 py-1 ">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-2 mb-2">
            ...
            <div class="text-center">
                <div class="text-md text-sm sm:text-lg font-bold text-gray-900">{{ reviewsStats.average }} ({{ reviewsStats.total }})</div>
        ...
        <div v-if="userStore.user?.id && !userHasReviewed" class="mb-2  px-4 rounded-lg">
            <ReviewForm :service-id="serviceId" @review-submitted="handleReviewSubmitted" />
        </div>
        ...
        <div v-if="reviews.length > 0" class="space-y-2">
            <div v-for="review in reviews" :key="review.id" class="border-b border-gray-100 pb-2 last:border-0">
                <div class="flex items-start space-x-3">
                    <img :src="review.user.avatar || '/default-avatar.png'" :alt="review.user.nom"
                    ...
                    <div class="flex-1">
                        ...
                        <div v-if="review.provider_response"
                            ...
                            <div class="flex items-start space-x-3">
                                ...
                                <div>
                                    <h5 class="font-semibold text-green-800 mb-1">Réponse du prestataire</h5>
...
import ReviewForm from './FormReview.vue';
...
const getRatingPercentage = (rating: number) => {
    const total = reviewsStats.value.total || 1;
...
const fetchReviews = async (page = 1) => {
    try {
        ...
        const response = await apiClient.get(`/services/${props.serviceId}/getReviews`, {
            params: { page }
        });
...
// Watcher pour recharger quand le service change
