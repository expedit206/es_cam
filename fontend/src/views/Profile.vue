<template>
  <SeoHead :title="t('my_profile')" :description="t('profile_seo_description')" :keywords="t('profile_seo_keywords')"
    url="https://espacecameroun.com/profil" type="profile" />
  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white shadow-sm border-b">
      ...
      <div class="container mx-auto px-4 py-8">
        <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          ...
          <div class="relative">
            <div class="w-28 h-28 rounded-2xl border-4 border-white bg-white shadow-lg overflow-hidden">
              <img v-if="user?.photo" :src="storageUrl + user.photo" :alt="user.nom"
            ...
            <button @click="openPhotoMenu"
              class="absolute -bottom-2 -right-2 w-8 h-8 bg-[var(--espace-or)] text-[var(--espace-vert)] rounded-full flex items-center justify-center shadow-lg hover:bg-[var(--espace-vert)] hover:text-white transition-all">
          ...
          <div class="text-center sm:text-left space-y-3 flex-1">
            ...
            <div class="flex flex-wrap items-center justify-center sm:justify-start gap-4 text-sm text-gray-600">
              <div v-if="user?.ville" class="flex items-center gap-1">
                <i class="fas fa-map-marker-alt"></i>
                <span>{{ user.ville }}{{ user?.quartier ? ', ' + user.quartier : '' }}</span>
              </div>
    ...
    <div class="container mx-auto px-4 py-8">
      <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6">
          <h2 class="text-xl font-semibold text-gray-900 mb-6">{{ t('personal_information') }}</h2>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
              ...
              <div class="space-y-4">
                <h3 class="font-medium text-gray-900 flex items-center gap-2">
                  <i class="fas fa-id-card text-[var(--espace-vert)]"></i>
                  {{ t('identity') }}
                </h3>
                <div class="space-y-3">
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-gray-600">{{ t('full_name') }}</span>
                    <span class="font-medium text-gray-900">{{ user?.nom }}</span>
                  </div>
                  <div v-if="userAge" class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-gray-600">{{ t('age') }}</span>
                    <span class="font-medium text-gray-900">{{ userAge }} ans</span>
                  </div>
                  <div v-if="user?.date_naissance"
                    class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-gray-600">{{ t('birth_date') }}</span>
                    <span class="font-medium text-gray-900">{{ formatDate(user.date_naissance) }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                ...
                <div class="space-y-3">
                  <div v-if="user?.ville" class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-gray-600">{{ t('city') }}</span>
                    <span class="font-medium text-gray-900">{{ user.ville }}</span>
                  </div>
                  <div v-if="user?.quartier" class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-gray-600">{{ t('neighborhood') }}</span>
                    <span class="font-medium text-gray-900">{{ user.quartier }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="space-y-6">
              ...
              <div class="space-y-4">
                <h3 class="font-medium text-gray-900 flex items-center gap-2">
                  <i class="fas fa-address-book text-[var(--espace-vert)]"></i>
                  {{ t('contact') }}
                </h3>
                <div class="space-y-3">
                  <div v-if="user?.email" class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-gray-600">{{ t('email') }}</span>
                    <span class="font-medium text-gray-900">{{ user.email }}</span>
                  </div>
                  <div v-if="user?.telephone" class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-gray-600">{{ t('phone') }}</span>
                    <span class="font-medium text-gray-900">{{ user.telephone }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                <h3 class="font-medium text-gray-900 flex items-center gap-2">
                  <i class="fas fa-user-circle text-[var(--espace-vert)]"></i>
                  {{ t('account') }}
                </h3>
                <div class="space-y-3">
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-gray-600">{{ t('member_since') }}</span>
                    <span class="font-medium text-gray-900">{{ memberSinceFull }}</span>
                  </div>
                  <div class="flex justify-between items-center py-2 border-b border-gray-100">
                  ...
                  <div v-if="user?.parrainage_code"
                    class="flex justify-between items-center py-2 border-b border-gray-100">
                    <span class="text-gray-600">{{ t('referral_code') }}</span>
                    <span class="font-medium text-gray-900 font-mono">{{ user.parrainage_code }}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6 mt-6">
          <h2 class="text-xl font-semibold text-gray-900 mb-6">{{ t('security') }}</h2>
          ...
          <div class="space-y-4">
            <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h3 class="font-medium text-gray-900">{{ t('account_protection') }}</h3>
                  <p class="text-gray-600 text-sm">{{ t('account_protection_desc') }}</p>
                </div>
              </div>
              <span :class="[
        ...
        <!-- Section parrainage -->
        <div v-if="user?.parrainage_code" class="bg-white rounded-xl shadow-sm border p-6 mt-6">
          <h2 class="text-xl font-semibold text-gray-900 mb-4">{{ t('referral_program') }}</h2>
          ...
          <div class="bg-gradient-to-r from-green-50 to-blue-50 rounded-lg p-6">
            <div class="flex items-center gap-4">
              ...
              <div class="flex-1">
                <h3 class="font-semibold text-gray-900">{{ t('your_referral_code') }}</h3>
                <p class="text-gray-600 text-sm mt-1 mb-3">
                  {{ t('referral_description') }}
                </p>
                <div class="flex items-center gap-3">
                  <div class="flex-1 bg-white border border-gray-300 rounded-lg px-4 py-3">
                    <code class="text-lg font-mono text-gray-800 font-bold">{{ user.parrainage_code }}</code>
                  </div>
                  <button @click="copyReferralCode"
                    class="bg-[var(--espace-vert)] text-white px-6 py-3 rounded-lg hover:bg-green-700 transition-colors flex items-center gap-2 font-medium">
    ...
    <!-- Preview photo -->
    <div v-if="previewUrl" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl p-6 max-w-md w-full">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('new_profile_picture') }}</h3>
        <div class="flex flex-col items-center space-y-4">
          <img :src="previewUrl" :alt="t('preview')" class="w-32 h-32 rounded-xl object-cover shadow-md" />
          <div class="flex gap-3 w-full">
            <button @click="uploadProfilePicture" :disabled="uploading"
              class="flex-1 bg-[var(--espace-vert)] text-white py-2 px-4 rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
    ...
    <!-- Camera Modal -->
    <div v-if="showCameraModal" class="fixed inset-0 bg-black/90 flex items-center justify-center z-50">
      <div class="bg-white rounded-2xl p-6 w-full max-w-md mx-4">
        <h3 class="text-lg font-semibold mb-4 text-center">
          {{ t('take_photo') }}
        </h3>
        <video ref="videoRef" autoplay playsinline class="rounded-xl w-full bg-black" />
