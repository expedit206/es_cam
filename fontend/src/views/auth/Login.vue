...
<template>
    <div class="overflow-y-scroll min-h-full flex justify-center w-full py-5 pb-24">
        <div class="max-w-md rounded-3xl shadow-0xl border-0 border-white/20 p-8 space-y-8 pb-24 block">
            <!-- En-tête avec logo -->
            <div class="text-center space-y-4">
                <div class="flex justify-center">
                    <img src="/src/assets/images/logo/LOGO_vert_all.png" alt="" class="w-1/2">
                </div>
                <div>
            ...
            <form @submit.prevent="login" class="space-y-6">
                <!-- Champ login -->
                <FormField :label="t('auth_phone_number') " icon="fa-phone" v-model="credentials.login" type="text" required
                    :placeholder="t('auth_phone_number')" :error="errors.login" autocomplete="tel" />
                ...
                <div class="space-y-2">
                    <div class="flex justify-between items-center">
                        ...
                        <button type="button" @click="toggleForgotPassword"
                            class="text-xs text-[var(--espace-vert)] hover:underline font-medium transition-colors">
                            {{ t('forgot_password') }}
                        </button>
                    </div>
                    <div class="relative">
                        <input :type="showPassword ? 'text' : 'password'" v-model="credentials.mot_de_passe" required
                            :placeholder="t('password')" :class="[
                                'w-full px-4 py-3 pl-12 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[var(--espace-vert)] focus:border-transparent transition-all duration-200',
                                errors.mot_de_passe ? 'border-red-500 focus:ring-red-500' : 'focus:ring-[var(--espace-vert)]'
                    ...
                    <!-- Message d'erreur -->
                    <p v-if="errors.mot_de_passe" class="text-red-500 text-xs mt-1 flex items-center gap-1">
                ...
                <button type="submit" :disabled="isLoading"
                    class="w-full bg-[var(--espace-vert)] text-white font-semibold py-4 rounded-xl hover:shadow-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-md">
            ...
            <div v-if="showForgotPassword"
                ...
                <div class="bg-white rounded-2xl p-6 w-full max-w-md space-y-4">
                    <div class="flex justify-between items-center">
                        <h3 class="text-lg font-semibold text-gray-900">{{ t('forgot_password') }}</h3>
                    ...
                    <div class="space-y-3">
                        <label class="block text-sm font-medium text-gray-700">{{ t('email_address') }}</label>
                        <input v-model="forgotPasswordEmail" type="email" :placeholder="t('email_placeholder')"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[var(--espace-vert)] focus:border-transparent transition-all duration-200"
                            required />
                    </div>
                    ...
                    <div class="flex gap-3 pt-2">
                        ...
                        <button @click="sendPasswordReset" :disabled="isSendingReset || !forgotPasswordEmail.trim()"
                            class="flex-1 py-3 bg-[var(--espace-vert)] text-white rounded-xl font-medium hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                            <div class="flex items-center justify-center gap-2">
                                <i v-if="isSendingReset" class="fas fa-spinner fa-spin"></i>
                                <span>{{ isSendingReset ? t('sending') : t('send_reset_link') }}</span>
                            </div>
                        </button>
                    </div>
                </div>
            ...
            <div class="space-y-4 text-center pb-6">
                ...
                <p class="text-sm text-gray-600">
                    {{ t('no_account') }}
                    <router-link to="/register" class="text-[var(--espace-vert)] font-semibold hover:underline ml-1">
                        {{ t('signup_here') }}
                    </router-link>
                </p>
                ...
                <p class="text-sm text-gray-600">
                    {{ t('visitor_access') }}
                    <router-link to="/home" class="text-[var(--espace-vert)] font-semibold hover:underline ml-1">
                        {{ t('visitor') }}
                    </router-link>
                </p>
            </div>
