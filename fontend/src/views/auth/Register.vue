<template>
    <div class="h-full  flex items-center justify-center p-4 overflow-y-scroll ">
        <div class="relative w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 h-full ">
            <!-- Section Gauche - Barre Latérale (Desktop uniquement) -->
            <div class="hidden md:block space-y-6 md:space-y-8 text-center md:text-left pt-8">
                <!-- Logo et Titre -->
                <div class="space-y-4">
                    <div class="flex items-center justify-center md:justify-start space-x-3">
                        <div
                            class="w-12 h-12 bg-gradient-to-r from-[var(--espace-vert)] to-green-600 rounded-2xl flex items-center justify-center shadow-lg">
                            <i class="fas fa-store text-white text-xl"></i>
                        </div>
                        <h1 class="text-3xl font-bold text-gray-900">{{ t('platform_name') }}</h1>
                    </div>
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-white/20">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <h3 class="font-semibold text-gray-900">{{ t('language_selector') }}</h3>
            ...
            <div class="relative md:hidden space-y-6 text-center ">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-center space-x-3">
                        <div
                            class="w-10 h-10 bg-gradient-to-r from-[var(--espace-vert)] to-green-600 rounded-2xl flex items-center justify-center shadow-lg">
                            <i class="fas fa-store text-white text-lg"></i>
                        </div>
                        <h1 class="text-2xl font-bold text-gray-900">{{ t('platform_name') }}</h1>
                    </div>
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-2xl p-4 shadow-xl border border-white/20">
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                            <i class="fas fa-globe text-blue-600 text-sm"></i>
                        </div>
                        <div>
                            <h3 class="font-semibold text-gray-900 text-sm">{{ t('language_selector') }}</h3>
            ...
            <div class="relative overflow-y-scrol">
                ...
                <div
                    ...
                    <!-- Éléments décoratifs -->
                    <div
                        class="absolute top-0 right-0 w-24 h-24 md:w-32 md:h-32 bg-gradient-to-br from-[var(--espace-or)]/10 to-transparent rounded-full -translate-y-12 md:-translate-y-16 translate-x-12 md:translate-x-16">
                    </div>
                    <div
                        class="absolute bottom-0 left-0 w-20 h-20 md:w-24 md:h-24 bg-gradient-to-tr from-[var(--espace-vert)]/10 to-transparent rounded-full translate-y-10 md:translate-y-12 -translate-x-10 md:-translate-x-12">
                    </div>
                    ...
                    <div class="relative z-10">
                        ...
                        <form @submit.prevent="handleSubmit" class="space-y-5 md:space-y-6">
                            <!-- Nom complet -->
                            <FormField :label="t('full_name')" icon="fa-user" v-model="form.nom" type="text" required
                                :placeholder="t('full_name')" :error="errors.nom" autocomplete="name"
                                @blur="validateField('nom')" class="text-sm md:text-base" />

                            <!-- Numéro de téléphone -->
                            <FormField :label="t('phone_number')" icon="fa-phone" v-model="form.telephone" type="tel"
                                required :placeholder="t('phone_number')" :error="errors.telephone"
                                pattern="\+?[0-9]{8 ,12}" autocomplete="tel" @blur="validateField('telephone')"
                            ...
                            <div class="space-y-3">
                                <div class="relative">
                                    <FormField :label="t('password')" icon="fa-lock" v-model="form.mot_de_passe"
                                        :type="showPassword ? 'text' : 'password'" required
                                        :placeholder="t('password_placeholder')" :error="errors.mot_de_passe"
                                        autocomplete="new-password" @input="validatePassword"
                                        class="text-sm md:text-base" />
                                    <button type="button"
                                ...
                                <div v-if="form.mot_de_passe" class="space-y-2">
                                    ...
                                    <div class="w-full bg-gray-200 rounded-full h-1 md:h-1.5">
                                        <div class="h-1 md:h-1.5 rounded-full transition-all duration-300"
                            ...
                            <div class="bg-gray-50 rounded-xl p-3 md:p-4"
                                ...
                                <label class="flex items-start space-x-3 cursor-pointer">
                                    <input type="checkbox" v-model="acceptTerms"
                                        class="mt-1 w-4 h-4 text-[var(--espace-vert)] border-gray-300 rounded focus:ring-[var(--espace-or)]">
                                    <span class="text-xs md:text-sm text-gray-600">
                                        {{ t('accept_terms') }}
                                        <router-link to="/politique-utilisation"
                                            class="text-[var(--espace-vert)] hover:underline font-medium">
                                            {{ t('terms_of_use') }}
                                        </router-link>
                                        {{ t('and') }}
                                ...
                                <p v-if="errors.terms" class="text-red-500 text-xs mt-2 flex items-center space-x-1">
                            ...
                            <button type="submit" :disabled="!canSubmit || isLoading"
                                class="w-full py-3 md:py-4 bg-gradient-to-r from-[var(--espace-vert)] to-green-600 text-white rounded-xl font-semibold hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm md:text-base">
                                <i :class="[isLoading ? 'fas fa-spinner fa-spin' : 'fas fa-rocket', 'mr-2']"></i>
                        ...
                        <div class="relative my-6 md:my-8">
                            ...
                            <div class="relative flex justify-center text-xs md:text-sm">
                                <span class="px-2 bg-white text-gray-500">{{ t('or_faster') }}</span>
                            </div>
                        </div>

                        <!-- Connexion sociale -->
                        <button @click="registerWithGoogle" :disabled="isGoogleLoading"
                            ...
                            <span>{{ isGoogleLoading ? t('google_redirect') : t('continue_with_google') }}</span>
                        </button>
                        ...
                        <p class="text-center text-xs md:text-sm text-gray-600 mt-4 md:mt-6">
                            ...
                            <router-link to="/login"
                                class="text-[var(--espace-vert)] font-semibold hover:underline ml-1">
                                {{ t('login_here') }}
                            </router-link>
                        </p>
                    </div>
...
const canSubmit = computed(() => {
    return form.value.nom.length >= 2 &&
...
const validateField = (field: string) => {
    switch (field) {
        case 'nom':
            ...
            break;
        case 'telephone':
            if (!form.value.telephone || !/\+?[0-9]{9,12}/.test(form.value.telephone)) {
