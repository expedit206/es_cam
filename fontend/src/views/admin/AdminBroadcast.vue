    ...
    <div class="max-w-2xl mx-auto bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
      <div class="mb-6 bg-blue-50 text-blue-800 p-4 rounded-xl flex items-start gap-3">
        ...
        <div>
          ...
          <p class="text-sm">Ce message sera envoyé individuellement à tous les utilisateurs enregistrés.
            Lorsqu'un utilisateur répondra, sa réponse apparaîtra dans votre messagerie privée habituelle.</p>
        </div>
      </div>
      ...
      <form @submit.prevent="sendBroadcast" class="space-y-6">
        <!-- Message Type -->
        <div>
          <label class="block text-gray-700 font-bold mb-2">Type de message</label>
          <div class="flex gap-4">
            <label class="flex items-center gap-2 cursor-pointer p-3 rounded-xl border" :class="messageType === 'text' ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'">
              <input type="radio" v-model="messageType" value="text" class="text-[var(--espace-vert)] focus:ring-[var(--espace-vert)]">
              <i class="fas fa-comment-alt"></i> Texte
            </label>
            <label class="flex items-center gap-2 cursor-pointer p-3 rounded-xl border" :class="messageType === 'image' ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'">
              <input type="radio" v-model="messageType" value="image" class="text-[var(--espace-vert)] focus:ring-[var(--espace-vert)]">
        ...
        <div v-if="messageType === 'image'" class="space-y-2">
          ...
          <div class="border-2 border-dashed border-gray-300 rounded-2xl p-6 text-center cursor-pointer hover:bg-gray-50 transition-colors relative" @click="$refs.fileInput.click()">
             ...
             <div v-if="imagePreview" class="relative inline-block">
                ...
                <button @click.stop="clearImage" class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs shadow-md">
...
const sendBroadcast = async () => {
    if (!confirm('Êtes-vous sûr de vouloir envoyer ce message à TOUS les utilisateurs ?')) return;
    ...
    try {
        ...
        toast.success(`Message envoyé avec succès (${response.data.count} destinataires)`);
