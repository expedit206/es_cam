  ...
  <div class="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="text-center mb-16 animate-fade-in">
        <h1 class="text-4xl md:text-5xl font-bold text-gray-900 mb-4 font-heading">
      ...
      <!-- Categories Grid -->
      <div v-if="marketplaceStore.isLoading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        <div 
          ...
          class="group relative h-72 rounded-3xl overflow-hidden cursor-pointer shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border border-gray-100"
        >
          <!-- Category Image -->
          ...
          <div class="absolute bottom-0 left-0 right-0 p-6 text-white text-center">
            <div class="mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-y-4 group-hover:translate-y-0">
               <span class="inline-block p-2 rounded-full bg-white/20 backdrop-blur-md mb-2">
                 <i :class="[category.icon, 'text-xl']"></i>
               </span>
            </div>
            <h3 class="text-2xl font-bold mb-1 font-heading">{{ category.name }}</h3>
            <p class="text-sm text-gray-300 opacity-0 group-hover:opacity-100 transition-all duration-500">
