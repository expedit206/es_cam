                ...
                <div class="bg-white rounded-2xl shadow-lg p-6 sm:p-8 max-w-md mx-auto">
                    ...
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <h2 class="text-xl font-bold text-gray-900">
                            Invité par
                            <span class="text-green-600 italic">{{ userEstFilleul.parrain_nom }}</span>
                        </h2>
                    </div>

                    <p class="text-gray-600 mb-6">
                        Vérifiez que vous n'êtes pas un robot et gagnez
                        <span class="text-green-600 font-semibold">{{ bonusParrain }} jetons</span>
                    </p>
                    ...
                    <div v-if="!verificationEnvoyee">
                        <div class="mb-4">
                            ...
                            <div class="relative">
                                ...
                                <input v-model="emailVerification" type="email"
                                    ...
                                    placeholder="Entrez votre email" :class="{ 'border-red-500': emailError }">
                            </div>
                            <p v-if="emailError" class="mt-1 text-sm text-red-600">{{ emailError }}</p>
                        </div>

                        <button @click="demanderVerification" :disabled="!emailValide || loadingVerification"
                    ...
                    <div v-else>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="relative">
                                <input v-model="codeVerification" type="text" maxlength="6"
                                    ...
                                    placeholder="000000" :class="{ 'border-red-500': codeError }" @input="formatCode">
                                <button v-if="codeVerification.length === 6" @click="verifierEmail"
                                    ...
                                    <i v-if="loadingVerification" class="fas fa-spinner fa-spin w-4 h-4"></i>
            ...
            <div v-if="userEstFilleul && userEstFilleul.email_verifie" class="mb-8">
                <div class="bg-white rounded-2xl shadow-lg p-8 max-w-md mx-auto text-center">
                    ...
                    <p class="text-gray-600 mb-4">
                        Vous avez reçu <span class="text-green-600 font-semibold">{{ userEstFilleul.bonus_parrain }}
                            jetons</span>
                    </p>
            ...
            <div v-if="userEstParrain" class="space-y-8">
                ...
                <div class="border-b-2 p-4">
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2">
                        <div>
                            <h2 class="text-md md:text-2xl font-bold text-gray-900 mb-1">Mon Réseau de Parrainage</h2>
                            <p class="text-gray-600 text-sm md:text-xl">Partagez votre lien et gagnez des jetons</p>
                        </div>
                        <div class="flex-1 max-w-lg">
                            <p class="text-sm font-medium text-gray-700 mb-1">Lien de parrainage :</p>
                            <div class="flex flex-col sm:flex-row gap-3">
                                <!-- Lien avec copie -->
                                <div class="flex gap-2 flex-1">
                                    <code class="flex-1 bg-gray-100 px-2 py-2 rounded-lg text-xs border truncate">
                        {{ lienParrainage }}
                    </code>
                                    <button @click="copierLien"
                                        ...
                                        <i class="fas" :class="copieReussie ? 'fa-check' : 'fa-copy'"></i>
                                        <span class="hidden sm:inline">{{ copieReussie ? 'Copié !' : 'Copier' }}</span>
                                    </button>
                                </div>

                                <!-- Bouton de partage natif -->
                                <button v-if="partageSupporté" @click="partagerLien"
                            ...
                            <div v-if="!partageSupporté" class="mt-3 flex flex-wrap gap-2">
                                <button v-for="reseau in reseauxSociaux" :key="reseau.nom"
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-2">
                    <div class="bg-white rounded-xl shadow-lg p-1  ">
                        <div class="flex items-center flex flex-col">
                                <div class="text-lg font-bold text-gray-900">{{ stats.total_bonus || 0 }}</div>
                                <div class="text-xs text-gray-500">Filleuls total</div>
                        </div>
                    </div>

                    <div class="bg-white rounded-xl shadow-lg p-1  ">
                        <div class="flex items-center flex flex-col">
                                <div class="text-lg font-bold text-yellow-800">{{ stats.en_attente || 0 }}</div>
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-1  ">
                        <div class="flex items-center flex flex-col">
                                <div class="text-lg font-bold text-gray-900">{{ stats.actifs || 0 }}</div>
                                <div class="text-xs text-gray-500">Bonus activés</div>
                            </div>
                    </div>

                    <div class="bg-white rounded-xl shadow-lg p-1  ">
                        <div class="flex items-center flex flex-col">
                                <div class="text-lg font-bold text-green-900">{{ stats.total_bonus || 0 }}</div>
                                <div class="text-xs text-gray-500">Tokens gagnés</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg px-2 pb-8">
                    ...
                    <div v-if="parrainages.length === 0" class="text-center py-12">
                        ...
                        <p class="text-gray-400 text-sm">Partagez votre lien de parrainage pour inviter des amis</p>
                    </div>

                    <div v-else class="grid gap-4">
                        <div v-for="parrainage in parrainages" :key="parrainage.id"
                            ...
                            <div class="flex items-start gap-1">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <span
                                        ...
                                        :class="{
                                            'bg-yellow-100 text-yellow-800': parrainage.statut === 'en_attente',
                                            'bg-green-100 text-green-800': parrainage.statut === 'bonus_attribue',
                                            'bg-red-100 text-red-800': parrainage.statut === 'rejected'
                                        }">
                                    ...
                                    <p class="text-gray-600 text-sm mb-1">
                                        ...
                                        <span v-if="parrainage.email_verification" class="text-gray-400">
                                ...
                                <div class="text-right flex-shrink-0">
                                    <div v-if="parrainage.bonus_attribue" class="text-green-600 font-semibold">
...
const reseauxSociaux = [
    {
        ...
        url: (lien, texte) => `https://wa.me/?text=${encodeURIComponent(texte + ' ' + lien)}`
    },
    {
        nom: 'Facebook',
        icone: 'fab fa-facebook',
        classes: 'border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100',
        url: (lien, texte) => `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(lien)}&quote=${encodeURIComponent(texte)}`
    },
    {
        nom: 'Twitter',
        icone: 'fab fa-twitter',
        classes: 'border-sky-200 bg-sky-50 text-sky-700 hover:bg-sky-100',
        url: (lien, texte) => `https://twitter.com/intent/tweet?text=${encodeURIComponent(texte)}&url=${encodeURIComponent(lien)}`
    },
    {
        nom: 'Email',
        icone: 'fas fa-envelope',
        classes: 'border-gray-200 bg-gray-50 text-gray-700 hover:bg-gray-100',
        url: (lien, texte) => `mailto:?subject=${encodeURIComponent('Rejoins-moi sur Espace Cameroun')}&body=${encodeURIComponent(texte + '\n\n' + lien)}`
    }
];
...
const demanderVerification = async () => {
    ...
    try {
        ...
        emailError.value = error.response?.data?.message || 'Erreur lors de l\'envoi du code';
...
const verifierEmail = async () => {
    ...
    try {
        ...
        codeError.value = error.response?.data?.message || 'Erreur lors de la vérification';
