<template>
    <div class="min-h-screen bg-gray-50 relative">
                   ...
                   <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
                        ...
                        <div
                            ...
                            <div class="flex-1">
                                ...
                                <div v-if="false && promotionActive"
                                    ...
                                    <div class="flex items-center gap-1"><i class="fas fa-heart"></i> <span>{{
                                            produitCounts?.favorites_count || 0 }} favoris</span></div>
                                    <div class="flex items-center gap-1"><i class="fas fa-comment-dots"></i> <span>{{
                                            produitCounts?.contacts_count || 0 }} contacts</span></div>
                                    <div class="flex items-center gap-1"><i class="fas fa-share-alt"></i> <span>{{
                                            produitCounts?.partages_count || 0 }} partages</span></div>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                            <div>
                                ...
                                <p class="text-gray-700 text-sm sm:text-base font-bold">{{ formatPrice(produit?.prix) }}
                                </p>
                            </div>
                            <div>
                        ...
                        <div v-if="false && isProductOwner && promotionActive"
                            ...
                            <div>
                                <div class="flex justify-between text-sm mb-1">
                                    <span class="text-gray-600">Taux de conversion (clic → contact)</span>
                                    <span class="font-medium text-gray-900">{{ calculateConversionRate() }}%</span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-green-500 h-2 rounded-full"
                                        :style="{ width: calculateConversionRate() + '%' }"></div>
                                </div>
                            </div>
                            ...
                            <div v-if="promotionActive"
                                ...
                                <div class="flex justify-between items-center text-sm mb-2 flex-wrap gap-2">
                                    <div>
                                        ...
                                        <span class="font-bold ml-1">{{ promotion?.used_clicks || 0 }}/{{
                                            promotion?.total_clicks }}</span>
                                    </div>
                                    <div class="text-right">
                                        <span class="text-blue-700">Coût total:</span>
                                        <span class="font-bold ml-1">{{ promotion?.total_cost || 0 }} jetons</span>
                                    </div>
                                </div>
                                ...
                                <div class="mt-2 w-full bg-blue-100 rounded-full h-1.5">
                                    <div class="bg-gradient-to-r from-blue-500 to-indigo-600 h-1.5 rounded-full"
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="text-gray-600 text-xs truncate">
                                        Membre depuis {{ formatMemberSince(produit?.user?.created_at) }}
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <div class="p-6">
                    ...
                    <div v-if="promotionActive" class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="bg-blue-50 rounded-lg p-4 text-center">
                                <p class="text-xl font-bold text-blue-900">{{ promotion?.used_clicks || 0 }}</p>
                                <p class="text-sm text-blue-700">Clics utilisés</p>
                            </div>
                            <div class="bg-green-50 rounded-lg p-4 text-center">
                                <p class="text-xl font-bold text-green-900">{{ promotion?.remaining_clicks || 0 }}</p>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            <div class="flex justify-between text-sm mb-2">
                                ...
                                <span class="font-bold text-gray-900">{{ getPromotionProgress() }}%</span>
                            </div>
                            <div class="w-full bg-gray-200 rounded-full h-3">
                                <div class="bg-gradient-to-r from-green-500 to-emerald-600 h-3 rounded-full"
                                    :style="{ width: getPromotionProgress() + '%' }"></div>
                            </div>
                        ...
                        <div class="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    <p class="text-lg font-bold text-gray-900">{{ promotion?.total_cost || 0 }} jetons
                                    </p>
                                    <p class="text-xs text-gray-500">{{ (promotion?.total_cost || 0) * 100 }} FCFA</p>
                                </div>
                                <div class="text-right">
                    ...
                    <div v-else class="space-y-6">
                        ...
                        <div
                            ...
                            <div class="flex justify-between items-center mb-3">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-gray-900">{{ userTokens }} jetons</p>
                                    <p class="text-xs text-gray-500">{{ userTokens * 100 }} FCFA</p>
                                </div>
                                <button @click="openBuyTokensModal"
                            ...
                            <p class="text-xs text-gray-600">
                                <i class="fas fa-info-circle mr-1"></i> Coût fixe: 0.1 jetons par clic (20 FCFA)
                            </p>
                        </div>

                        <!-- Sélection du nombre de clics -->
                        <div>
                            <h4 class="font-semibold text-gray-900 mb-4">Combien de clics souhaitez-vous acheter ?</h4>

                            <!-- Options rapides -->
                            <div class="grid grid-cols-3 gap-3 mb-4">
                                <button v-for="option in quickClicOptions" :key="option.clicks"
                                    @click="selectedClicks = option.clicks" :class="[
                                        'border rounded-lg p-3 text-center transition-all',
                                        selectedClicks === option.clicks
                                            ? 'border-blue-500 bg-blue-50'
                                            : 'border-gray-200 hover:border-gray-300'
                                    ]">
                                    <p class="text-lg font-bold text-gray-900">{{ option.clicks }}</p>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex items-center">
                                    <input v-model.number="customClicks" type="number" min="10" max="10000"
                                ...
                                <p class="text-xs text-gray-500 mt-1">Minimum: 10 clics, Maximum: 10,000 clics</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="border-t border-gray-300 pt-2 mt-2">
                                    <div class="flex justify-between font-bold">
                                        ...
                                        <span class="text-lg text-blue-600">{{ totalCost }} jetons</span>
                                    </div>
                                    <p class="text-sm text-gray-500 text-right">{{ totalCost * 100 }} FCFA</p>
                                </div>
                            ...
                            <div class="mt-4 p-3 bg-blue-50 rounded-lg">
                                <p class="text-sm text-blue-700">
                                    ...
                                    <span class="font-bold">{{ Math.round(totalClicks * 0.05) }}-{{
                                        Math.round(totalClicks * 0.1) }}</span>
                                    contacts potentiels.
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex space-x-3 pt-2">
                            ...
                            <button @click="createPromotion" :disabled="!canAffordPromotion || totalClicks < 10" :class="[
        ...
        <div v-if="showAddClicks"
            class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div class="bg-white rounded-lg max-w-md w-full p-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-bold text-gray-900">Ajouter des clics</h3>
                    <button @click="showAddClicks = false" class="text-gray-400 hover:text-gray-600">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input v-model.number="additionalClicks" type="number" min="10" max="500"
                    ...
                    <div class="bg-gray-50 p-3 rounded-lg">
                        <div class="flex justify-between text-sm mb-1">
                            ...
                            <span class="font-bold">{{ additionalClicks * promotion?.cost_per_click }} jetons</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-600">Nouveau total de clics</span>
                            <span class="font-bold">{{ (promotion?.total_clicks || 0) + additionalClicks }} clics</span>
                        </div>
                    </div>

                    <div class="flex space-x-3">
                        <button @click="showAddClicks = false"
                            class="flex-1 px-4 py-2 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200">
                            Annuler
                        </button>
                        <button @click="addMoreClicks"
        ...
        <div v-if="showBuyTokensModal"
            ...
            <div class="bg-white rounded-lg max-w-md w-full p-6">
                ...
                <div class="space-y-4">
                    <div v-for="option in tokenOptions" :key="option.tokens" @click="selectedTokenOption = option"
                        :class="[
                        ...
                        <div class="flex justify-between items-center">
                            <div>
                                <p class="text-lg font-bold text-gray-900">{{ option.tokens }} jetons</p>
                                <p class="text-sm text-gray-600">{{ option.tokens * 100 }} FCFA</p>
                            </div>
                            <div v-if="option.bonus" class="text-right">
                                <p class="text-sm text-green-600 font-medium">+{{ option.bonus }} bonus</p>
                                <p class="text-xs text-gray-500">Total: {{ option.tokens + option.bonus }} jetons</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <p class="text-xs text-gray-500 text-center mt-2">
                        Vous recevrez {{ selectedTokenOption.tokens + (selectedTokenOption.bonus || 0) }} jetons
                    </p>
                </div>
            </div>
        </div>
        ...
        <div v-if="showPromotionConfirmation"
            class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div class="bg-white rounded-lg max-w-sm w-full p-6 text-center">
                <div class="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                    <i class="fas fa-check text-green-600 text-xl"></i>
                </div>
                <h3 class="text-lg font-semibold text-gray-900 mb-2">Campagne lancée !</h3>
                <p class="text-sm text-gray-600 mb-4">
                    Votre campagne PPC est maintenant active. Vous serez débité uniquement lorsque des utilisateurs
                    cliqueront sur votre produit.
                </p>
                <button @click="showPromotionConfirmation = false"
...
const stopPromotion = async () => {
    ...
    try {
        const response = await apiClient.post(`/promotions/${promotion.value.id}/stop`);
...
const seoTitle = computed(() => {
    return produit.value ? `${produit.value?.nom} - Produits | Espace Cameroun` : 'Produit | Espace Cameroun';
});

const seoDescription = computed(() => {
    return produit.value?.description?.substring(0, 160) || 'Découvrez ce produit de qualité sur Espace Cameroun';
});
...
const fetchProduitCounts = async () => {
    ...
    try {
        const response = await apiClient.get(`/produits/${produit.value.id}/counts`);
...
const fetchPromotion = async () => {
    ...
    try {
        const response = await apiClient.get(`/promotions/active/${produit.value.id}`);
...
const fetchPromotionStats = async () => {
    ...
    try {
        const response = await apiClient.get(`/promotions/${promotion.value.id}/stats`);
        if (response.data.success) {
...
const addMoreClicks = async () => {
    ...
    const additionalCost = additionalClicks.value * promotion.value.cost_per_click;
    ...
    try {
        const response = await apiClient.post(`/promotions/${promotion.value.id}/add-clicks`, {
...
const calculateConversionRate = () => {
    if (!produitCounts.value?.clics_count || produitCounts.value.clics_count === 0) return 0;
    const rate = (produitCounts.value.contacts_count / produitCounts.value.clics_count) * 100;
    return Math.min(rate, 100).toFixed(1);
};

const calculateAverageCPC = () => {
    if (!promotion.value?.used_clicks || promotion.value.used_clicks === 0) return 0;
...
const proceedToPayment = async () => {
    ...
    try {
        ...
        if (response.data.success) {
            // Mettre à jour les jetons
            user.jetons += selectedTokenOption.value.tokens + (selectedTokenOption.value.bonus || 0);
...
const toggleFavorite = async () => {
    ...
    try {
        const response = await apiClient.post(`/produits/${produit.value.id}/favorite`);
...
watch(isProductOwner, (newValue) => {
    if (newValue) {
        fetchPromotion();
    }
}, { immediate: true }); // immediate: true pour exécuter immédiatement
