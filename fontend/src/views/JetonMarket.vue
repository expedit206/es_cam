<template>
    ...
    <div class="h-full overflow-y-auto bg-gradient-to-b from-gray-50 to-gray-100 pb-24">
        <Loader :isLoading="isLoading" />
        <!-- Header fixe -->
        <div class="fixed top-0 left-0 right-0 z-50 bg-white border-b shadow-sm px-4 py-1">
            <div class="flex items-center justify-between">
                <button @click="$router.back()" class="p-2 text-gray-800 hover:text-[var(--espace-vert)]">
                    <i class="fas fa-arrow-left text-lg"></i>
                </button>

                <div class="text-center">
                    <h1 class=" text-md md:text-lg font-bold text-black">{{ t('token_market') }}</h1>
        ...
        <div class="pt-16 px-4">
            <!-- Bannière principale -->
            <div
                class="relative overflow-hidden  rounded-2xl p-5 text-[var(--espace-vert)] mb-6 shadow-xl border-2 border-black">
                ...
                <Transition name="fade">
                    <div v-if="showEducation"
                        ...
                        <!-- Comment gagner -->
                        <div>
                            <h4 class="font-semibold text-[var(--espace-vert)] mb-3 flex items-center">
                                <i class="fas fa-coins mr-2 text-[var(--espace-vert)]"></i>
                                {{ t('how_to_earn_tokens') }}
                            </h4>
                            <ul class="space-y-2 px-2">
                                <li class="flex items-start gap-2">
                                    <div class="rounded-full w-5 h-5 flex items-center justify-center">
                                        <i class="fas fa-user-plus text-black text-xs"></i>
                                    </div>
                                ...
                                <li class="flex items-start gap-2">
                                    <div class="rounded-full w-5 h-5 flex items-center justify-center">
                                        <i class="fas fa-calendar text-black text-xs"></i>
                                    </div>
                                    <span class="text-black">
                                        {{ t('special_events') }}
                                    </span>
                                ...
                                <li class="flex items-start gap-2">
                                    <div class=" rounded-full w-5 h-5 flex items-center justify-center">
                                        <i class="fas fa-sync-alt text-black text-xs"></i>
                                    </div>
                                    <span class="text-black">
                                        {{ t('secondary_market') }}
                                    </span>
                        ...
                        <div>
                            <h4 class="font-semibold text-[var(--espace-vert)] mb-3 flex items-center">
                                <i class="fas fa-rocket mr-2 text-[var(--espace-vert)]"></i>
                                {{ t('use_tokens_for') }}
                            </h4>
                            <ul class="space-y-2 px-2">
                                <li class="text-black">
                        ...
                        <!-- Marché des Jetons -->
                        <div class="bg-white/10 backdrop-blur-md rounded-xl border border-white/10">
                            <h4 class="font-semibold text-[var(--espace-vert)] mb-3 flex items-center">
                                <i class="fas fa-chart-bar mr-2 text-[var(--espace-vert)]"></i>
                                {{ t('market_allows') }}
                            </h4>
                            <ul class="space-y-2 px-2">
                                <li class="text-black"><strong>{{ t('invest_growth') }}</strong></li>
                                <li class="text-black"><strong>{{ t('monetize_tokens') }}</strong></li>
                                <li class="text-black"><strong>{{ t('exchange_community') }}</strong></li>
                                <li class="text-black"><strong>{{ t('optimize_strategy') }}</strong></li>
                            </ul>
                        </div>
            ...
            <div v-if="user" class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 mb-4">
                <div class="grid grid-cols-2 gap-4 text-center">
                    <div>
                        <div class="text-2xl font-bold text-[var(--espace-vert)]">{{ user.jetons || 0 }}</div>
                    ...
                    <div>
                        <div class="text-2xl font-bold text-black">{{ userWallets.length }}</div>
                        <div class="text-xs text-gray-600 flex items-center justify-center gap-1">
                    ...
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-3">
                            <router-link :to="getProfileRoute(offer)"
                                ...
                                <img v-if="offer.user.photo" :src="storageUrl + offer.user.photo"
                                    :alt="t('profile_picture')" class="w-full h-full object-cover">
                                <i v-else class="fas fa-user text-[var(--espace-vert)] text-sm"></i>
                            </router-link>
                            <router-link :to="getProfileRoute(offer)"
                                class="text-sm font-bold text-gray-800 truncate hover:text-[var(--espace-vert)] hover:underline max-w-[100px]">
                                {{ offer.user.nom }}
                            </router-link>
                        </div>
                        <span
                            class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-black border border-green-200">
                    ...
                    <div class="space-y-2 mb-4">
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-600 flex items-center gap-1">
                                <i class="fas fa-tag text-[var(--espace-vert)]"></i>
                                {{ t('unit_price') }}
                            </span>
                            <span class="font-bold text-[var(--espace-vert)]">{{ offer.prix_unitaire }} FCFA</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="font-bold text-gray-900">{{ formatPrice(offer.total_prix) }} FCFA</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between text-xs">
                            ...
                            <span class="text-gray-500">{{ formatDate(offer.date_creation) }}</span>
                        </div>
                    </div>

                    <!-- Bouton d'achat -->
                    <button @click="openPaymentModal(offer.id)"
                        class="w-full border-2 text-[var(--espace-vert)] px-3 py-3 rounded-xl font-bold shadow-lg hover:from-yellow-500 hover:to-yellow-600 hover:shadow-xl transition-all duration-200 active:scale-95 flex items-center justify-center gap-2 text-sm">
            ...
            <div v-else class="text-center py-12">
                <div class="bg-white rounded-2xl p-8 shadow-lg border border-gray-100">
                    <div class="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                        <i class="fas fa-coins text-[var(--espace-vert)] text-2xl"></i>
                    </div>
                    ...
                    <button @click="checkWalletsBeforeOffer"
                        class="bg-gradient-to-r from-black to-[var(--espace-vert)] text-white px-6 py-3 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all active:scale-95">
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg z-100">
            <div class="flex items-center justify-around p-0 max-w-md mx-auto">
                <button @click="closeAllModals(); showFilters = true"
                    class="flex flex-col items-center p-2 text-gray-600 hover:text-[var(--espace-vert)] transition">
                    <i class="fas fa-filter text-lg mb-1"></i>
                    <span class="text-xs">{{ t('filters') }}</span>
                </button>

                <button @click="refreshOffers"
                    class="flex flex-col items-center p-2 text-gray-600 hover:text-[var(--espace-vert)] transition"
                    :class="{ 'text-[var(--espace-vert)]': refreshing }">
                    <i class="fas fa-sync-alt text-lg mb-1" :class="{ 'fa-spin': refreshing }"></i>
                    <span class="text-xs">{{ t('refresh') }}</span>
                </button>

                <button @click="closeAllModals(); showWalletManager = true"
                    class="flex flex-col items-center p-2 text-gray-600 hover:text-[var(--espace-vert)] transition">
                    <i class="fas fa-wallet text-lg mb-1"></i>
                    <span class="text-xs">{{ t('wallet') }}</span>
                </button>

                <button @click="checkWalletsBeforeOffer"
                    class="flex flex-col items-center p-2 text-[var(--espace-vert)] hover:text-[var(--espace-vert)] transition">
                    <i class="fas fa-plus-circle text-lg mb-1"></i>
                    <span class="text-xs">{{ t('sell') }}</span>
                </button>

                <button @click="showOfferManager = true"
                    class="flex flex-col items-center p-2 text-gray-600 hover:text-[var(--espace-vert)] transition">
                    <i class="fas fa-list text-lg mb-1"></i>
                    <span class="text-xs">{{ t('my_offers') }}</span>
                </button>
            </div>
        </div>

        <!-- Modal Filtres -->
        <div v-if="showFilters" class="fixed  inset-0 bg-black/50 flex items-center  z-50">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-filter mr-2 text-[var(--espace-vert)]"></i>
                            {{ t('min_quantity') }}
                        </label>
                        <input v-model.number="filters.quantityMin" type="number" min="0"
                            :placeholder="t('example_quantity')"
                            class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                    </div>

                    <button @click="applyFilters"
                        class="w-full bg-[var(--espace-vert)] text-white py-3 rounded-lg font-semibold hover:bg-[var(--espace-vert)] transition flex items-center justify-center gap-2">
        ...
        <div v-if="showOfferForm" class="fixed inset-0 bg-black/50 flex items-end z-150">
            <div class="bg-white w-full rounded-t-2xl p-6 max-h-[90vh] overflow-y-auto">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-lg font-bold text-gray-900">
                        <i class="fas fa-coins text-yellow-500 mr-2"></i>
                        {{ t('sell_tokens') }}
                    </h2>
                    <button @click="showOfferForm = false" class="text-gray-400 hover:text-gray-600">
                        <i class="fas fa-times text-lg"></i>
                    </button>
                </div>
                ...
                <form @submit.prevent="submitOffer" class="space-y-4">
                    <div>
                        ...
                        <input v-model.number="offerForm.nombre_jetons" type="number" min="1"
                            :placeholder="t('example_quantity')" required
                            class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                        <p class="text-xs text-gray-500 mt-1">
                            {{ t('available_tokens') }} <strong>{{ user?.jetons || 0 }}</strong>
                        </p>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-[var(--espace-vert)] mb-2">
                            <i class="fas fa-tag text-[var(--espace-vert)] mr-2"></i>
                            {{ t('unit_price_cfa') }}
                        </label>
                        <input v-model.number="offerForm.prix_unitaire" type="number" min="1"
                            :placeholder="t('example_price')" required
                            class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                    </div>
                    ...
                    <div>
                        ...
                        <select v-model="offerForm.wallet_id" required
                            class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                    ...
                    <div class="bg-gray-50 rounded-xl p-4 space-y-3 border border-gray-200">
                        <h4 class="font-semibold text-gray-800 text-sm flex items-center gap-2">
                            <i class="fas fa-calculator text-[var(--espace-vert)]"></i>
                            {{ t('offer_summary') }}
                        </h4>
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                <span class="text-gray-600">{{ t('total_sale') }}</span>
                                <span class="font-semibold">{{ formatPrice(totalPrice) }} FCFA</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">{{ t('commission') }}</span>
                                <span class="font-semibold text-red-600">- {{ formatPrice(commission) }} FCFA</span>
                            </div>
                            <div class="flex justify-between border-t pt-2">
                                <span class="font-semibold text-gray-800">{{ t('you_will_receive') }}</span>
                                <span class="font-bold text-[var(--espace-vert)]">{{ formatPrice(totalPrice -
                                    commission) }} FCFA</span>
                            </div>
                        </div>
                    </div>

                    <button type="submit" :disabled="loading || offerForm.nombre_jetons > (user?.jetons || 0)"
                        class="w-full bg-gradient-to-r from-black to-[var(--espace-vert)] text-white py-3 rounded-lg font-semibold hover:from-[var(--espace-vert)] hover:to-black disabled:opacity-50 transition flex items-center justify-center gap-2">
        ...
        <div v-if="showPaymentModal" class="fixed inset-0 bg-black/50 flex items-end z-150">
            <div class="bg-white w-full rounded-t-2xl p-6">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-lg font-bold text-gray-900">
                        <i class="fas fa-shopping-cart text-[var(--espace-vert)] mr-2"></i>
                        {{ t('confirm_purchase') }}
                    </h2>
                ...
                <form @submit.prevent="confirmPayment" class="space-y-4">
                    ...
                    <div class="bg-blue-50 rounded-xl p-4 border border-blue-200">
                        <h4 class="font-semibold text-blue-900 mb-3">{{ t('offer_details') }}</h4>
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                <span class="text-blue-700">{{ t('seller') }}</span>
                                <span class="font-semibold">{{ selectedOffer?.user?.nom }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-blue-700">{{ t('tokens_quantity') }}</span>
                                <span class="font-semibold">{{ selectedOffer?.nombre_jetons }} jetons</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-blue-700">{{ t('unit_price') }}</span>
                                <span class="font-semibold">{{ formatPrice(selectedOffer?.prix_unitaire || 0) }}
                                    FCFA</span>
                            </div>
                            <div class="flex justify-between border-t pt-2">
                                <span class="font-semibold text-blue-900">{{ t('total_amount') }}</span>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-envelope text-[var(--espace-vert)] mr-2"></i>
                            {{ t('email_address') }} *
                        </label>
                        <input v-model="paymentDetails.email" type="email" :placeholder="t('email_placeholder')"
                            required
                            class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--espace-vert)] focus:border-transparent">
                    </div>
                    ...
                    <div class="bg-green-50 border border-green-200 rounded-xl p-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h5 class="font-semibold text-green-900 text-sm">{{ t('secure_payment') }}</h5>
                                <p class="text-xs text-green-700">{{ t('notchpay_redirection_message') }}</p>
                            </div>
                        </div>
                    </div>

                    <button type="submit" :disabled="loading || !canSubmitPayment"
                        class="w-full bg-gradient-to-r from-black to-[var(--espace-vert)] text-white py-3 rounded-lg font-semibold hover:from-[var(--espace-vert)] hover:to-black disabled:opacity-50 transition flex items-center justify-center gap-2">
                        <i v-if="loading" class="fas fa-spinner fa-spin"></i>
...
const structuredData = computed(() => {
    ...
    return {
        ...
        data: {
            ...
            "description": t('token_market_description'),
            "url": `${window.location.origin}${router.currentRoute.value.path}`,
            "mainEntity": offerItems.length > 0 ? {
                "@type": "ItemList",
...
const canSubmitPayment = computed(() => {
    return paymentDetails.value.email &&
        selectedOffer.value &&
...
const confirmPayment = async () => {
    ...
    // Validation des champs NotchPay
    if (!paymentDetails.value.email || !isValidEmail(paymentDetails.value.email)) {
    ...
    try {
        ...
        const response = await apiClient.post(`/jeton/market/buy/${selectedOffer.value.id}`, {
